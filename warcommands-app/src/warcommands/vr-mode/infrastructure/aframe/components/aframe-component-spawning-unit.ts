...
export class AFrameComponentSpawningUnit {
    ...
    constructor(
        ...
        AFRAME.registerComponent(this.componentName, {
            ...
            buildSpawningSphere: function(geometry: THREE.SphereGeometry, color: number, side: Side) {
                const materialOutside = new THREE.MeshStandardMaterial( {color: color, side: side} );
