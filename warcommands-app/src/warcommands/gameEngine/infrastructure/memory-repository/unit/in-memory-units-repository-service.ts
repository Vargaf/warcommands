...
export class InMemoryUnitsRepositoryService implements UnitsRepositoryService {
    ...
    findById(unitId: string): UnitGenericDTO {
        return <UnitGenericDTO>_.cloneDeep(this.unitsList.get(unitId));
    }
    ...
    findBy(filterList: QueryFilterDTO): UnitGenericDTO[] {
        ...
        this.unitsList.forEach((unit: UnitGenericDTO) => {
            
            let filtersMatch = true;

            for (const filterKey of filterKeyList) {
                const objectKey = filterKey as keyof UnitGenericDTO;
                if (unit[objectKey] !== filterList[objectKey]) {
                    filtersMatch = false;
                    break;
                }
            }
